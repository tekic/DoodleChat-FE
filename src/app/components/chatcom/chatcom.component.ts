...
export class ChatcomComponent implements OnInit {
  ...
  constructor(private chatsrvService: ChatsrvService, private toastr: ToastrService
    ) { }
  ...
  sendMessageUsingRest() {
    if (this.form.valid) {
      let message: Message = { message: this.form.value.message, fromId: this.userForm.value.fromId, toId: this.userForm.value.toId };
      this.chatsrvService.post(message).subscribe(res => {
        console.log(res);
      })
    }
  }
  ...
  handleResult(message){
    if (message.body) {
      ...
      this.toastr.success('<h2> New message! </h2>', null, {
        'timeOut': 3000,
        'enableHtml' :  true
      });
    }
